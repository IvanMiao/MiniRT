# **************************************************************************** #
#                                                                              #
#                                                         :::      ::::::::    #
#    Makefile                                           :+:      :+:    :+:    #
#                                                     +:+ +:+         +:+      #
#    By: ymiao <ymiao@student.42.fr>                +#+  +:+       +#+         #
#                                                 +#+#+#+#+#+   +#+            #
#    Created: 2025/08/11 21:57:02 by ymiao             #+#    #+#              #
#    Updated: 2025/09/05 05:59:49 by ymiao            ###   ########.fr        #
#                                                                              #
# **************************************************************************** #

CC		= clang
CFLAG	= -Wall -Werror -Wextra -O3

NVCC	= nvcc
NVFLAGS = -O3 -arch=sm_75

NAME	= miniRT

SRC_DIR	= src/
SRC		= main.c \
			math_tool/vector_part1.c math_tool/vector_part2.c \
			math_tool/color.c \
			objects/objects_list.c \
			objects/sphere.c \
			objects/plane.c \
			objects/cylinder.c \
			objects/hit.c \
			parsing/assignment_amb.c \
			parsing/assignment_obj.c \
			parsing/assignment_cam.c \
			parsing/utils_atof.c \
			parsing/utils_parse.c \
			parsing/parser.c \
			event/event.c \
			render/camera.c \
			render/ray.c \
			render/shadow.c \
			render/all_lights.c \
			render/render.c \
			utils/mem_manager.c \
			utils/ft_split.c \
			utils/ft_split_space.c \
			utils/ft_putstr_fd.c \
			utils/ft_strcmp.c \
			utils/ft_atoi.c \
			utils/ft_isspace.c \
			utils/ft_isdigit.c \
			utils/ft_error.c \
			utils/get_next_line.c \
			utils/get_next_line_utils.c

SRC_CU	= render/render_cuda.cu

OBJ_DIR	= obj/
OBJ		= $(addprefix $(OBJ_DIR),$(SRC:.c=.o))
OBJ_CU	= $(addprefix $(OBJ_DIR),$(SRC_CU:.cu=.o))

CUDA_LIB_NAME = libcuda_render.a
CUDA_LIB = $(OBJ_DIR)$(CUDA_LIB_NAME)

MLX_URL	= https://cdn.intra.42.fr/document/document/34994/minilibx-linux.tgz
MLX_TAR	= mlx.tar.gz

MLX_LIBS = -Lmlx_linux -lmlx_Linux -L/usr/lib -lXext -lX11 -lm -lz
CUDA_RUNTIME_LIBS = -lcudart
MY_LIBS = -L$(OBJ_DIR) -lcuda_render
INCLUDES = -I/usr/include -Imlx_linux -I$(SRC_DIR)

all : check_mlx $(NAME)

check_mlx:
	@if [ ! -d "mlx_linux" ]; then \
		echo "Mlx library not found. Downloading..."; \
		wget $(MLX_URL) -O $(MLX_TAR); \
		echo "Extracting Mlx library..."; \
		tar -xzf $(MLX_TAR); \
		mv minilibx-linux mlx_linux; \
		echo "Configuring mlx library..."; \
		cd mlx_linux && bash configure; \
		rm -f ../$(MLX_TAR); \
	fi

bonus:
	cd bonus && make

$(NAME): $(OBJ) $(CUDA_LIB)
	$(CC) $(CFLAG) -o $(NAME) $(OBJ) $(MLX_LIBS) $(MY_LIBS) $(CUDA_RUNTIME_LIBS)

$(CUDA_LIB): $(OBJ_CU)
	$(NVCC) $(NVFLAGS) -lib -o $@ $(OBJ_CU)

$(OBJ_DIR)%.o: $(SRC_DIR)%.c
	@mkdir -p $(dir $@)
	$(CC) $(CFLAG) -I/usr/include -Imlx_linux -c $< -o $@

$(OBJ_DIR)%.o: $(SRC_DIR)%.cu
	@mkdir -p $(dir $@)
	$(NVCC) $(NVFLAGS) $(INCLUDES) -c $< -o $@

clean :
	rm -rf $(OBJ_DIR)

fclean : clean
	rm -f $(NAME)
	cd bonus && make fclean

re : fclean all

.PHONY : all clean fclean re check_mlx bonus
