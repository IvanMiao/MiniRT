...
__device__ static t_color calculate_diffuse_light_gpu(t_light light, t_vector hit_point,
...
__device__ t_color combine_light_gpu(const t_minirt_gpu *scene, const t_hit_record *hit)
{
	...
	// 检查阴影并计算漫反射光
	if (!is_in_shadow_gpu(scene, hit->point, scene->light.position))
    {
		diffuse_color = calculate_diffuse_light_gpu(scene->light, hit->point,
				hit->normal, hit->obj_color);
    }
	else
    {
		diffuse_color = (t_color){0.0f, 0.0f, 0.0f};
    }
...
extern "C" void render_with_cuda(t_minirt *rt)
{
    ...
    device_scene.light = host_scene.light;
